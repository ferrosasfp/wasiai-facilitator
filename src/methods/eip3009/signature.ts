 ...
 *   - This module lives under `src/methods/eip3009/`.
 *   - Runtime imports only `src/core/errors.ts` (excepción [1]) and viem types.
 *   - Type-only import of `src/core/types.ts`.
 *
 * Spec references:
 ...
 *   { ok: false, error: buildX402Error('INVALID_SIGNATURE', <reason>) }
 *
...
export function normalizeSignature(sig: Hex): NormalizedSignature | SignatureError {
  ...
  if (typeof sig !== 'string' || !sig.startsWith('0x')) {
    return {
      ok: false,
      error: buildX402Error('INVALID_SIGNATURE', 'invalid signature length'),
    };
  }
  ...
  if (rBigInt === null) {
    return {
      ok: false,
      error: buildX402Error('INVALID_SIGNATURE', 'invalid signature hex (r)'),
    };
  }
  ...
  if (sRawBigInt === null) {
    return {
      ok: false,
      error: buildX402Error('INVALID_SIGNATURE', 'invalid signature hex (s)'),
    };
  }
  ...
  if (body.length === 128) {
    ...
    if (vNum === null) {
      return {
        ok: false,
        error: buildX402Error('INVALID_SIGNATURE', 'invalid signature hex (v)'),
      };
    }
    ...
    if (vNum === 0n || vNum === 27n) {
      ...
      return {
        ok: false,
        error: buildX402Error('INVALID_SIGNATURE', `invalid signature v value: ${vNum.toString()}`),
      };
    }
    sBigInt = sRawBigInt;
  }
  ...
  if (rBigInt === 0n || sBigInt === 0n) {
    return {
      ok: false,
      error: buildX402Error('INVALID_SIGNATURE', 'signature r or s is zero'),
    };
  }
  ...
  if (rBigInt >= SECP256K1_N || sBigInt >= SECP256K1_N) {
    return {
      ok: false,
      error: buildX402Error('INVALID_SIGNATURE', 'signature r or s out of range'),
    };
  }
  ...
  if (sBigInt > SECP256K1_N_HALF) {
    return {
      ok: false,
      error: buildX402Error('INVALID_SIGNATURE', 'signature s-value in high half (malleable)'),
    };
  }
